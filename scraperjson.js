/**
 * ScorePop — Mackolik → JSON Scraper
 * v5: Firebase kaldırıldı, JSON çıktı eklendi.
 *
 * Kullanım:
 ...
 *   --outDir=./data   JSON çıktı klasörü (varsayılan: ./data)
 */
...
function parseStandingsHtml(html) {
    ...
    while ((row = rowRegex.exec(html)) !== null) {
        ...
        standings.push({
            ...
            played, win, draw, lose, points, gf: 0, ga: 0, gd: 0,
        });
    }
    return standings;
}
...
async function fetchMatchH2H(matchId) {
    ...
    try {
        ...
        if (raw.includes('Object moved') || raw.includes('PageError.htm')) {
            log(`  ⚠️ H2H redirect matchId=${matchId}`);
...
(async () => {
    try {
        if (MODE === 'daily') {
            ...
            await processDate(parseTargetDate(SINGLE));
